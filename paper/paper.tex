...
a ten-second IPerf session, which involves transferring as much data as possible
over a TCP connection. Ten seconds is the default interval for an IPerf session.
In our experiments, the TCP flows reached their peak throughput in under a
second. Thus, the ten second interval is sufficient to observe the steady-state
throughput behavior of the TCP flows.
