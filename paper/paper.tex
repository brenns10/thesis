...
problem. It is designed with the following goals in mind \cite{rfc6824}.
...
+---------------+            +-------------------------------+
\end{BVerbatim}
  \caption[Comparison of TCP and MPTCP Protocol Stacks]{Comparison of Standard
    TCP and MPTCP Protocol Stacks. Originally from \cite{rfc6824}}
  \label{fig:layers}
\end{figure}

To achieve these goals, MPTCP is located between TCP and the application layer,
...
\item The \emph{detour} is a host which acts as a waypoint along the overlay
  route from the client to the server. It tunnels the MPTCP between them. There
...
manager extension for the Linux MPTCP implementation, which allows the client's
kernel to request new detour paths, receive them, and create subflows across
them. Second, a user-space utility which receives messages from the kernel and
creates the necessary detour paths. Finally, we implement two different types of
detour daemons, which run on the detour host. These components are illustrated
in Figure~\ref{f:MovingParts}.
...
creates a MPTCP connection to the server. The first subflow uses the Internet
...
client reports this to the path-managers, which creates subflows for the MPTCP
...
We have implemented two different mechanisms for tunneling MPTCP subflows across
...
To enable subflows to use the detours created above, we implement a custom MPTCP
...
The path manager receives space within the MPTCP control buffer in order to
...
\texttt{detour}) runs a worker when a client-side MPTCP connection becomes fully
...
the appropriate internal list. Then, it iterates over every MPTCP connection
